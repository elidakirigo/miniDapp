	...
	if(msg.event == "inited"){
		...
		createDB(function(){
			...
			loadAllFilters(function(filters){
				...
				addStartupGroup(filters,"other");
				addStartupGroup(filters,"personal");
				addStartupGroup(filters,"politics");
				addStartupGroup(filters,"religion");
				addStartupGroup(filters,"science");
						...
						if(!valid){
							...
							insertMessage(msg_category, msg_title, 
								...
								if(inserted){
									...
									//Did this user send the message
									if(from_user){
										//Do we need to set the notify tag
										newNotifyTopic(cattitleid);
